...
void sortbyposte()
{
    for (int i = 0; i < countPlayers - 1; i++)
    {
        for (int j = i + 1; j < countPlayers; j++)
        {
            if (stricmp(players[i].poste, players[j].poste) > 0)
            {
                Player temp = players[i];
                players[i] = players[j];
                players[j] = temp;
...
void triwithposte()
{
    ...
    do {
        ...
        printf("1. Filter by position\n");
        printf("2. Sort by position\n");
        printf("3. Back\n");
        printf("Choose: ");
        scanf("%d", &choice);
        ...
        switch(choice) {
            case 1:
                ...
                printf("Enter Position you want to filter by (Defender, Midfielder, Forward, Goalkeeper): ");
